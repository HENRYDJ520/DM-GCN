...
class Processor():
    ...
    def load_model(self):
        ...
        if self.arg.weights:
            self.global_step = int(arg.weights[:-3].split('-')[-2])
    ...
    def train(self, epoch, save_model=False):
        ...
        self.print_log('\tMean training loss: {:.4f}.  Mean training acc: {:.2f}%.\tLearning Rate: {:.4f}.\tTime consumption: [Data]{dataloader}, [Network]{model}'.format(np.mean(loss_value), np.mean(acc_value)*100, self.lr, **proportion))
        if save_model:
    ...
    def eval(self, epoch, save_score=False, loader_name=['test'], wrong_file=None, result_file=None):
        ...
        for ln in loader_name:
            ...
            #  打印在同一行
            log_msg = '\tMean {} loss of {} batches: {}.'.format(
                ln, len(self.data_loader[ln]), np.mean(loss_value))
            for k in self.arg.show_topk:
                log_msg += '\tTop{}: {:.2f}%'.format(
                    k, 100 * self.data_loader[ln].dataset.top_k(score, k))
            log_msg += '\tModel: {}'.format(self.arg.model)
