...
# parser.add_argument('--dataset', default='ntu/xsub', help='the work folder for storing results')
...
if 'ntu' in arg.dataset:
    if 'xsub' in arg.dataset:
        npz_data = np.load(r'C:\zangtuo\NTU60_XSub.npz')
        label = np.where(npz_data['y_test'] > 0)[1]
        print("60--------------------------------------------------------")
    elif 'xview' in arg.dataset:
        npz_data = np.load(r'C:\zangtuo\NTU60_XView.npz')
        label = np.where(npz_data['y_test'] > 0)[1]
elif '120' in arg.dataset:
    if 'xsub' in arg.dataset:
        npz_data = np.load(r'C:\zangtuo\NTU120_XSub.npz')
        label = np.where(npz_data['y_test'] > 0)[1]
        ...
        npz_data = np.load(r'C:\zangtuo\NTU120_XSet.npz')
        label = np.where(npz_data['y_test'] > 0)[1]
    ...
    npz_data = np.load(r'D:\jhr\HDN-GCNtrain_joint.npz')
    label = np.where(npz_data['y_test'] > 0)[1]
...
if __name__ == '__main__':
    ...
    pso.pso()
# python enu_two_graph.py --dataset 120/xsub --joint_dir two_person_sorce/hypergnn --bone_dir two_person_sorce/two_graph --joint_motion_dir two_person_sorce/alltransformer
